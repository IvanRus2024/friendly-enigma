...
class Library:
    def __init__(self, books: list[Book] | None = None):
        # Используем None как значение по умолчанию для избежания проблем с изменяемыми типами
        self.books = books.copy() if books is not None else []
    ...
    def get_index_by_book_id(self, book_id: int) -> int:
        ...
        for idx, book in enumerate(self.books):  # Используем enumerate для пар индекс-значение
            if book.id_ == book_id:
                return idx
